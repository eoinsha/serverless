...
module.exports = {
  checkForChanges() {
    console.log('checkForChanges', this.bucketName)
  ...
  getMostRecentObjects() {
    ...
    console.log('getMostRecentObjects', params)
  ...
  getObjectMetadata(objects) {
    if (objects && objects.length) {
      const headObjectObjects = objects
        .map((obj) => {
          console.log('HEAD OBJECT', this.bucketName, obj.Key);
          return this.provider.request('S3',
          'headObject',
          {
            Bucket: this.bucketName,
      ...
      return BbPromise.all(headObjectObjects)
        .then((result) => result)
        .catch(err => {
          console.error(err, 'ERROR in S3 HEAD OBJECT');
          throw err;
        })
    }
